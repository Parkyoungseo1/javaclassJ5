...
public class MemberDAO {
	...
	public MemberVO getMemberIdCheck(String mid) {
		...
		try {
			sql = "select * from junggomember where mid = ?";
	...
	public int setMemberJoinOk(MemberVO vo) {
		int res = 0;
		try {
			sql = "insert into junggomember values (default,?,?,?,?,?,?,?,?,?,?,?,?,default,default,default,default,default,default,default)";
			pstmt = conn.prepareStatement(sql);
	...
	public MemberVO getMemberNickCheck(String nickName) {
		MemberVO vo = new MemberVO();
		try {
			sql = "select * from junggomember where nickName = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public int setMemberPwdChange(String mid, String pwd) {
		int res = 0;
		try {
			sql = "update junggomember set pwd=? where mid = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<MemberVO> getMemberList() {
		...
		try {
			sql = "select * from junggomember order by idx";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public int setMemberDeleteUpdate(String mid) {
		int res = 0;
		try {
			sql = "update junggomember set userDel = 'OK', level=99 where mid = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mid);
			res = pstmt.executeUpdate();
		} catch (SQLException e) {
